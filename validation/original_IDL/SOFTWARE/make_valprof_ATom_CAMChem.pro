...
; modifications by Sara Martinez-Alonso 4.2017 to use SAMBBA data
; modifications by Sara Martinez-Alonso 8.2017 to use ACRIDICON-CHUVA data
; modifications by Sara Martinez-Alonso 12.2017 to use ACTAMERICA data
; modifications by Sara Martinez-Alonso 02.2018 to use ATom data

pro make_valprof_ATom_CAMChem
...
;;this is for SAMBBA files
;inputhead='/home/sma/Desktop/SAMBBA/RAW/sambba_'										;when reading from my own directories
;outputhead='/home/sma/Desktop/SAMBBA/INTERPOLATED_PROFILES/sambba_'							;when writting in my own directories

;;;this is for ATom files
;;;;;;;;CHANGE AS NEEDED;;;;;;;;;;;;;;;;;;;;;;
;inputhead='/home/sma/Desktop/ATom_FOR_MOPITT_VALIDATION/CO_NOAA/RAW/'										;when reading from my own directories
;outputhead='/home/sma/Desktop/ATom_FOR_MOPITT_VALIDATION/CO_NOAA/INTERPOLATED_PROFILES/'							;when writting in my own directories
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;;this is for ATom files
;;;;;;;;CHANGE AS NEEDED;;;;;;;;;;;;;;;;;;;;;;
;inputhead='/home/sma/ATom_08022019_RELEASE/CO_QCLS_filled/RAW/'										;when reading from my own directories
;outputhead='/home/sma/ATom_08022019_RELEASE/CO_QCLS_filled/INTERPOLATED_PROFILES/'							;when writting in my own directories
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;;this is for ATom files
;;;;;;;;CHANGE AS NEEDED;;;;;;;;;;;;;;;;;;;;;;
;inputhead= '/home/sma/Desktop/ATom_VS_TROPOMI/KATHRYNS_TBL_FILE_OUTPUT/NOAA_PICARRO/RAW/'
;outputhead='/home/sma/Desktop/ATom_VS_TROPOMI/KATHRYNS_TBL_FILE_OUTPUT/NOAA_PICARRO/INTERPOLATED_PROFILES/'
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;;this is for ATom files
;;;;;;;;CHANGE AS NEEDED;;;;;;;;;;;;;;;;;;;;;;
;inputhead='/home/sma/Desktop/FROM_ROISIN_10152019/Sara_merge/HYBRID_QCLS_PICARRO/RAW/'										;when reading from my own directories
;outputhead='/home/sma/Desktop/FROM_ROISIN_10152019/Sara_merge/HYBRID_QCLS_PICARRO/INTERPOLATED_PROFILES/'							;when writting in my own directories
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;this is for ATom files
;;;;;;;CHANGE AS NEEDED;;;;;;;;;;;;;;;;;;;;;;
inputhead='/home/sma/Desktop/MAPS_FOR_NEWMAN_WOFSY/ATom_2019-11-25/ATom-4/RAW/'										;when reading from my own directories
outputhead='/home/sma/Desktop/MAPS_FOR_NEWMAN_WOFSY/ATom_2019-11-25/ATom-4/INTERPOLATED_PROFILES/'							;when writting in my own directories
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; endif
 ...
 pres_prof=fltarr(nlev)
  ...
  inputfile=inputhead+'*.asc'
  ;
  psfile=outputhead+'profiles_interpolated.ps'
  device,/port,/inch,xoff=1,yoff=1,xsiz=6.5,ysiz=9,file=psfile, /color
  !p.font=0
  !p.multi=[0,3,2]
  !p.charsize=1.2
  ...
  for ifile = 0,nfiles-1 do begin
   ...
   line1=' '
   readf, 1, line1
   for kk=1, 2 do readf,1,sdum	;read header lines
   for i=0,ndata-1 do begin
    readf,1,sdum
    cols = strsplit(sdum,/extract)
    jul=cols[1]
    ymdhm=file_basename(cofile, '.asc')
    nada=strsplit(ymdhm, '_', /extract)
    if n_elements(nada) eq 3 then ymdhm=nada(2)
    if n_elements(nada) eq 4 then ymdhm=nada(3)
    if n_elements(nada) eq 5 then ymdhm=nada(4)
    yr=strmid(ymdhm,0,4)
    mon=strmid(ymdhm,5,2)
    day=strmid(ymdhm,8,2)
    lat[i] = cols[5]
    lon[i] = cols[6]
    pres_ac[i] = cols[8]
    co_ac[i] = cols[9]
   ...
   ac_lon=mean_lon_function(lon)	;mean(lon, /nan)		;use mean lon
   ac_lat=mean(lat, /nan)		;use mean lat
   prmin = Min(pres_ac, max=prmax, /nan)
   print,ac_lat,ac_lon,prmin,prmax
   hour = strmid(ymdhm,11,2)
   minutes = strmid(ymdhm,13,2)
;   hrmin = String(hour,minutes,format='(i2.2,i2.2)')
   date1 = yr+'-'+mon+'-'+day+'.'+hour+minutes		;hrmin
   print,date1
...
;   xyouts,!x.crange[1],0.15,String(ac_lat,ac_lon,format='(f5.0,"N,",f5.0,"E")'),align=1.,charsize=0.8
    ...
    xyouts,0.5,0.98,'ATom profiles'+' extrapolated with CAM-Chem/MERRA monthly mean',/norm,align=0.5,charsize=1.
    oplot,co_avg,moplev, psym=-4, symsize=0.9, thick=2, color=250
    print,date1,' ',mozdate(itim)
    outfile = outputhead+ymdhm+'_interpolated.txt'
    openw,2,outfile
    printf,2, cofile
    printf, 2, line1
