...
;modifications by Rebecca Buchholz 12.2025
...
; hip and nsa missing
  ...
  for isite = 0, nsites-1 do begin
    ...
    for ivalfile = 0, nvalfiles-1 do begin
      ...
      ;;;;;;;;;;;;;;; Validation V10 ;;;;;;;;;;;;
      outfile = '/home/buchholz/MOPITTv10/MOPITT_Validation/mopittv10_python/validation/validation_pairing/val_L2_v10.L2V19.9.2.' + sitecodes(isite) + '.' + profyr + profmo + profdy + '.' + profhr + profmn + '.50km.dat'
            ...
            if (psfc(ibox(j)) lt prs(0) and psfc(ibox(j)) gt prs(1)) then begin
              ...
              if (psfc(ibox(j)) lt prs(1) and psfc(ibox(j)) gt prs(2)) then begin
                ...
                if (V8mode ne 'T') then begin
                  ...
                  printf, 1, format='(4i4,3f8.1,4e11.3,f7.4,1x,e10.3,1x,e10.3,1x,e10.3,5i3,1x,f9.4,1x,f9.4)', pxsttrk(0,ibox(j)), pxsttrk(1,ibox(j)), isfc(ibox(j)), icld(ibox(j)), psfc(ibox(j)), sza(ibox(j)), dist, rtvcolm(0,ibox(j)), colm_sim, apcolm(ibox(j)), wvcolm(ibox(j)), dfs(ibox(j)), snr5a(ibox(j)), snr6a(ibox(j)), snr6r(ibox(j)), anom_flags(0:4,ibox(j)), anom_flags, missing_val
